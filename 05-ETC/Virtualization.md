# Virtualization 
## 💡 가상화가 무엇이고, 하이퍼바이저 가상화와 컨테이너 가상화는 차이점이 무엇인지 설명해 주세요.
**`가상화 (Virtualization)`**

- 가상화는 컴퓨팅 환경을 논리적으로 분리하거나 추상화하여 물리적인 하드웨어 리소스를 여러 가상 환경 또는 가상 머신으로 나누는 기술입니다.
- 주요 목적은 사용자 격리와 리소스 공유 및 관리를 통해 하드웨어 가용성을 개선하는 것입니다.
...
- 기본적으로 각 애플리케이션은 내부적으로 Cgroups(control groups)과 네임스페이스를 활용하여 격리되며, 독립된 네트워크 환경을 가지고 있어 다른 애플리케이션과의 간섭을 방지합니다. 도커 데몬은 이러한 기술을 활용하여 각 컨테이너를 고유한 논리적 공간에 쉽게 배치하고 관리하는 역할을 합니다.
